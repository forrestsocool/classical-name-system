...
class 姓名热度测试(unittest.TestCase):
    def test本地报告包含四个完整热门字表和热门名表(self):
        报告 = 读取姓名报告()["报告"]
        self.assertEqual([项目["统计年度"] for 项目 in 报告], [2018, 2019, 2020, 2021])
    ...
    def test二〇一八年数据包含图表中的热门字和热门名(self):
        报告 = 读取姓名报告()["报告"][0]
        self.assertEqual(报告["统计年度"], 2018)
        self.assertEqual(len(报告["热门字"]), 50)
        self.assertEqual(报告["热门字"][15], "博")
        self.assertEqual(报告["热门字"][37], "馨")
        self.assertEqual(报告["热门名"]["男"][0], {"名字": "浩宇"})
        self.assertEqual(报告["热门名"]["女"][0], {"名字": "梓涵"})

...
def 生成基础候选(
    ...
    for 行 in 来源行:
        ...
        if any(字 in 名字 for 字 in 过滤字):
            continue
        评分 = 行["quality_score"]
        # 文化方向必须成为主要排序因素；否则精选词组会因基础分较高，
        # 无论用户选择哪个方向都长期占据前几位，看起来像方向没有生效。
        方向命中数 = sum(字 in 方向关键词 for 字 in 名字)
        if 方向:
            if 行["direction"] in 方向:
                评分 += 50
            elif 方向命中数:
                评分 += 20 + 10 * 方向命中数
            else:
        ...
        评分 += 2 if 名字[0] in 方向关键词 or 名字[-1] in 方向关键词 else 0

...
def 统计古籍(路径: Path) -> dict:
    ...
    风险 = []
    结构检查 = {
        "预期标题数": 预期篇章数.get(路径.stem),
        "标题数一致": (
            路径.stem not in 预期篇章数
            or len(标题) == 预期篇章数[路径.stem]
        ),
    }
    if not 结构检查["标题数一致"]:
        风险.append(
            f"篇章标题数异常：预期 {结构检查['预期标题数']}，实际 {len(标题)}"
        )
    if 路径.stem == "周易":
        卦名标题 = [
            内容 for _, 内容 in 标题 if re.match(r"【\d{2}\. ", 内容.strip())
        ]
        卦号 = [int(re.match(r"【(\d{2})\. ", 内容.strip()).group(1)) for 内容 in 卦名标题]
        结构检查.update(
            {
                "卦名标题数": len(卦名标题),
                "卦号覆盖": sorted(set(卦号)) == list(range(1, 65)),
                "卦号无重复": len(卦号) == len(set(卦号)),
            }
        )
        if not 结构检查["卦号覆盖"] or not 结构检查["卦号无重复"]:
            风险.append("周易卦名标题不完整或重复")
    if 路径.stem == "楚辞" and "〈" in 正文:
        风险.append("正文仍含未清除的校注标记")
    if 路径.stem == "礼记" and ("■" in 正文 or "（上“侃”" in 正文):
        风险.append("正文仍含未还原的占位字符或编辑说明")
    if 替换字符数:
        风险.append("存在替换字符")
    if 控制字符数:
        风险.append("存在非换行控制字符")
    ...
    if 路径.stem == "周易" and 统计["干"]:
        try:
            from 脚本.核验周易字形 import 生成报告
        except ModuleNotFoundError:
            from 核验周易字形 import 生成报告
        字形统计 = 生成报告(路径)["统计"]
        if 字形统计["建议改为乾"] or 字形统计["待人工核验"]:
            风险.append("周易仍有未完成的乾/干字形核对")
    return {
        "文件": str(路径),
        "编码": 编码,
        "字节数": 路径.stat().st_size,

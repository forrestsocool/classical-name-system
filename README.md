...
网页滑卡回归（需要安装 `playwright`，并有 Edge 浏览器；先启动本地服务）：

```powershell
python 脚本/验证滑卡.py --地址 http://127.0.0.1:8000 --截图目录 构建产物/运行数据/滑卡验收
```

此脚本拦截接口并模拟慢响应与失败，不产生真实模型费用、不写入业务数据；覆盖滑动、收藏归属、预加载、刷新续看和条件切换。`验证网页.py` 兼容转到同一检查。首次获取仍受模型响应速度影响，快速滑完库存后也可能需要等待；预加载会产生下一批模型调用，最多维持约两批已准备名字。

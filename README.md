...
姓名报告热度资料不写入 SQLite，而是随代码发布在 `公安部姓名报告参考资料/公安部姓名报告热度.json`，服务启动后本地读取并缓存。这样不依赖外网，也不会把实时人口数据误称为报告数据。当前纳入 2019—2021 统计年度的全国新生儿热门字和热门名；2022、2023 年按本次核验未纳入未经官方原文确认的数据。

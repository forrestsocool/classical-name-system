...
- 首页改为连续滑卡：左滑跳过、右滑收藏，支持按钮和键盘方向键；可撤回上一次跳过。
- 姓氏、名字字数、固定字和避用字记在本机；同一浏览器会话缓存未看卡片，七天内刷新可续看。
- 首批展示后预加载下一批；队列不多于12张时按需补充，每页最多一个生成请求。后台失败保留现有卡片，提供手动重试，切到其他栏目或隐藏页面时不发起新预载。
- 每张卡片携带生成任务编号；右滑立即进入下一张，收藏异步保存，失败留在本地待同步队列，可手动或联网后重试。管理密钥和模型密钥不写入滑卡缓存。
- 读取本地公安部全国姓名报告参考资料；名字命中历史热门字或热门名时，展示统计年度、排名和人数提示，不把历史热度当作实时重名结果。
...
此脚本拦截接口并模拟慢响应与失败，不产生真实模型费用、不写入业务数据；覆盖滑动、收藏归属、预加载、刷新续看、条件切换、触摸滑动和热门提示。`验证网页.py` 兼容转到同一检查。首次获取仍受模型响应速度影响，快速滑完库存后也可能需要等待；预加载会产生下一批模型调用，最多维持约两批已准备名字。
